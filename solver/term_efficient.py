...
class Term(object):
    def __init__(self, wfs=None, bounds=None, cst=None):
        ...
        if wfs is None:
            ...
            self.wfs = wfs
        # # args are the variables of the weight functions
        # if args is None:
        #     self.args = []
        # else:
        #     self.args = args
        ...
        for bounds1 in self.bounds:
            for bounds2 in other.bounds:
                ...
                for variable in bounds1_set & bounds2_set:
                    new_bounds[variable] = [max(bounds1.get(variable)[0], bounds2.get(variable)[0]), min(bounds1.get(variable)[1], bounds2.get(variable)[1])]
                for variable in bounds2_set-bounds1_set:
                    new_bounds[variable] = bounds2.get(variable)
    ...
    def integrate(self):
        integrated = {wf: wf.integrate(*wf.free_symbols) for wf in set(self.wfs)}
        integral = [symbolic_to_numeric(integrated[self.wfs[i]], self.bounds[i])*self.cst[i] for i in range(len(self.wfs))]
